...
namespace TreniniDotNet.Application.Catalog.CatalogItems.CreateCatalogItem
{
    ...
    {
        public static IRollingStock FromInput(this IRollingStocksFactory factory, RollingStockInput input, IReadOnlyDictionary<Slug, IRailwayInfo> railways)
        {
            if (!railways.TryGetValue(Slug.Of(input.Railway), out var railwayInfo))
            {
                throw new ArgumentOutOfRangeException(nameof(railways), $"Unable to find the '{input.Railway}' railway for this rolling stock");
            }
