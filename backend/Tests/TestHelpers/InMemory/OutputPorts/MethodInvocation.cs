...
namespace TreniniDotNet.TestHelpers.InMemory.OutputPorts
{
    ...
    public readonly struct MethodInvocation<T> : IMethodInvocation
    {
        ...
        public void ShouldBeInvokedWithTheArgument(T expected)
        {
            ...
            _value.Should().BeEquivalentTo(expected);
        }
    ...
    public readonly struct MethodInvocation<T1, T2> : IMethodInvocation
    {
        ...
        public void ShouldBeInvokedWithTheArguments(T1 expected1, T2 expected2)
        {
            ...
            _value1.Should().Be(expected1);
            _value2.Should().Be(expected2);
        }
        ...
        public void ArgumentsAreNotNull()
        {
            ...
            _value1.Should().NotBeNull();
            _value2.Should().NotBeNull();
        }
    ...
    public static class AssertMethod
    {
        ...
        {
            if (method.IsInvoked != false) return;
            var because = $"Expected {method.Name}() to be invoked, it was never called{method.PrintContext()}";
            method.IsInvoked.Should().BeTrue(because);
        }
    }
}
